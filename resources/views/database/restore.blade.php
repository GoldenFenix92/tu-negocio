<x-app-layout>
    <x-slot name="header">
        <h2 class="fw-semibold fs-4 text-white m-0">
            <i class="bi bi-upload me-2"></i>{{ __('Restaurar Backup de Base de Datos') }}
        </h2>
    </x-slot>
        ...
        <div class="container">
            ...
            <div class="card">
                <div class="card-body">
                    ...
                    {{-- Archivos de backup disponibles en el servidor --}}
                        ...
                        <div class="mb-4">
                            ...
                            <div class="row row-cols-1 row-cols-md-2 row-cols-lg-3 g-3">
                                @foreach($availableBackups as $backup)
                                    <div class="col">
                                        <div class="card h-100 bg-body-secondary border-secondary backup-card" style="cursor: pointer;" onclick="openBackupModal('{{ addslashes($backup['name']) }}', '{{ $backup['type'] }}', '{{ $backup['path'] }}', '{{ number_format($backup['size'] / 1024, 2) }}', '{{ $backup['modified'] }}', '{{ isset($backup['tables']) ? count($backup['tables']) : 0 }}', '{{ isset($backup['total_records']) ? number_format($backup['total_records']) : 0 }}')">
                                            <div class="card-body">
                                                ...
                                                <h6 class="card-title text-truncate mb-2">{{ $backup['name'] }}</h6>
                                                ...
                                                <div class="small text-secondary mb-3">
                                                    ...
                                                    @if($backup['type'] === 'csv' && isset($backup['tables']))
                                                        <div class="d-flex align-items-center mb-1">
                                                            <i class="bi bi-table me-1"></i>
                                                            {{ count($backup['tables']) }} tablas
                                                        </div>
                                                        <div class="d-flex align-items-center">
                                                            <i class="bi bi-list-ol me-1"></i>
                                                            {{ number_format($backup['total_records']) }} registros
                                                        </div>
                                                    @endif
                                                </div>
                                                ...
                                                <div class="d-flex gap-2">
                                                    @if($backup['type'] === 'csv')
                                                        <button type="button" onclick="event.stopPropagation(); previewCsvContent('{{ addslashes($backup['path']) }}')" class="btn btn-success btn-sm flex-fill">
                                                            Ver CSV
                                                        </button>
                                                    @else
                                                        <button type="button" onclick="event.stopPropagation(); previewSqlContent('{{ addslashes($backup['path']) }}')" class="btn btn-primary btn-sm flex-fill">
                                                            Ver SQL
                                                        </button>
                                                    @endif
                                                    <button type="button" onclick="event.stopPropagation(); selectBackup('{{ addslashes($backup['path']) }}', '{{ $backup['type'] }}')" class="btn btn-outline-primary btn-sm flex-fill">
                                                        Seleccionar
                                                    </button>
                                                </div>
                                            </div>
                    ...
                    {{-- O subir nuevo archivo --}}
                    <div class="border-top pt-4 mt-4">
                        <h5 class="mb-3">O subir un archivo SQL manualmente</h5>

                        <form method="POST" action="{{ route('database.restore_process') }}" enctype="multipart/form-data" id="upload-form">
                            @csrf
                            ...
                            <div class="d-flex justify-content-between">
                                ...
                                <div class="d-flex gap-2">
                                    <button type="submit" formaction="{{ route('database.restore_backup') }}" class="btn btn-success">
    ...
    {{-- Modal para mostrar información del backup --}}
    ...
    {{-- Modal para preview de contenido --}}
    ...
    <script>
        ...
        function analyzeSqlBackup(path) {
            closeBackupModal();
            window.location.href = `{{ route('database.restore_show_content') }}?server_backup_file=${encodeURIComponent(path)}`;
        }

        function analyzeCsvBackup(path) {
            closeBackupModal();
            window.location.href = `{{ route('database.restore_process_csv_get') }}?file_path=${encodeURIComponent(path)}`;
        }
        ...
        function previewSqlContent(path) {
            ...
            // Hacer petición AJAX
            fetch(`{{ route('database.preview_sql_content') }}`, {
                method: 'POST',
                headers: {
        ...
        function previewCsvContent(path) {
            ...
            // Hacer petición AJAX
            fetch(`{{ route('database.preview_csv_content') }}`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                    'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').getAttribute('content')
