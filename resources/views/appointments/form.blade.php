<x-app-layout>
    <x-slot name="header">
        <h2 class="fw-semibold fs-4 text-white m-0">{{ isset($appointment) ? 'Editar Cita' : 'Crear Cita' }}</h2>
    </x-slot>
                    ...
                    <form action="{{ isset($appointment) ? route('appointments.update', $appointment) : route('appointments.store') }}" method="POST">
                            ...
                            <div class="col-lg-6">
                                ...
                                <div class="mb-3">
                                    ...
                                    <select id="item_select" class="form-select">
                                        ...
                                        @foreach($allItems as $item)
                                            <option value="{{ $item->id }}" data-type="{{ $item->type }}" data-price="{{ $item->price }}" data-sell_price="{{ $item->sell_price ?? $item->price }}" data-stock="{{ $item->stock ?? 'N/A' }}" data-duration="{{ $item->duration_minutes ?? 'N/A' }}">
                                                {{ $item->name }} ({{ ucfirst($item->type) }}) - ${{ number_format($item->price, 2) }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="mb-3">
                                    ...
                                    <select name="status" id="status" class="form-select">
                                        <option value="Pendiente" {{ (isset($appointment) && $appointment->estatus == 'Pendiente') ? 'selected' : '' }}>Pendiente</option>
                                        <option value="Anticipo" {{ (isset($appointment) && $appointment->estatus == 'Anticipo') ? 'selected' : '' }}>Anticipo</option>
                                        <option value="Pagado" {{ (isset($appointment) && $appointment->estatus == 'Pagado') ? 'selected' : '' }}>Pagado</option>
                                        <option value="Cancelado" {{ (isset($appointment) && $appointment->estatus == 'Cancelado') ? 'selected' : '' }}>Cancelado</option>
                                    </select>
                                </div>
                                ...
                                <div id="deposit_fields" class="d-none">
                                    <div class="mb-3">
                                        ...
                                        <select name="deposit_type" id="deposit_type" class="form-select">
                                            ...
                                            <option value="Efectivo" {{ (isset($appointment) && $appointment->deposit_type == 'Efectivo') ? 'selected' : '' }}>Efectivo</option>
                                            <option value="Transferencia" {{ (isset($appointment) && $appointment->deposit_type == 'Transferencia') ? 'selected' : '' }}>Transferencia</option>
                                        </select>
                                    </div>
                                    ...
                                    <div class="mb-3 d-none" id="deposit_amount_field">
                                        ...
                                        <div class="input-group">
                                            ...
                                            <input type="number" step="0.01" name="deposit_amount" id="deposit_amount" class="form-control" value="{{ old('deposit_amount', $appointment->deposit_amount ?? '') }}">
                                        </div>
                                    </div>

                                    <div class="mb-3 d-none" id="deposit_folio_field">
                                        <label for="deposit_folio" class="form-label">Folio de Transferencia</label>
                                        <input type="text" name="deposit_folio" id="deposit_folio" class="form-control" value="{{ old('deposit_folio', $appointment->deposit_folio ?? '') }}">
                                    </div>
                                </div>
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="datetime-local" name="appointment_datetime" id="appointment_datetime" class="form-control" value="{{ old('appointment_datetime', isset($appointment) ? $appointment->appointment_datetime->format('Y-m-d\TH:i') : '') }}">
                                </div>

                                <!-- Comentarios -->
                                <div class="mb-3">
                                    <label for="comments" class="form-label">Comentarios</label>
                                    <textarea name="comments" id="comments" rows="3" class="form-control">{{ old('comments', $appointment->comments ?? '') }}</textarea>
                                </div>
                            </div>
                        ...
                        <div class="d-flex justify-content-end gap-2 mt-4">
                            ...
                            <button type="submit" class="btn btn-primary">
                                <i class="bi bi-check-lg me-1"></i>{{ isset($appointment) ? 'Actualizar Cita' : 'Crear Cita' }}
                            </button>
                        </div>
                    </form>
                </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            @if(isset($appointment) && $appointment->client_id)
                clientIdSelect.value = "{{ $appointment->client_id }}";
            @endif
            ...
            @if(isset($appointment) && isset($selectedAppointmentItems))
                selectedItems = @json($selectedAppointmentItems);
                renderSelectedItems();
                updateTotalCost();
            @endif
