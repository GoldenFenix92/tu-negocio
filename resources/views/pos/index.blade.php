<x-app-layout>
            ...
            @if($activeSession)
                <div class="d-flex align-items-center gap-3">
                    <span class="badge bg-success-subtle text-success border border-success rounded-pill px-3 py-2">
                        <i class="bi bi-cash-stack me-1"></i> Sesión Activa: ${{ number_format($activeSession->initial_cash ?? 0, 2) }}
                    </span>
                    </span>
                    <a href="{{ route('cash_sessions.show', $activeSession) }}" class="text-primary small">
                        Ver Sesión
                    </a>
                </div>
            @else
            ...
            <div class="product-grid">
                @foreach($products as $product)
                    <div class="product-card"
                        ...
                        data-item-price="{{ $product->sell_price }}"
                        ...
                        <div class="fw-semibold small lh-sm">{{ $product->name }}</div>
                        <div class="text-secondary extra-small">${{ number_format($product->sell_price, 2) }}</div>
                    </div>
                @endforeach
            </div>
            ...
            <div class="product-grid">
                @foreach($services as $service)
                    <div class="product-card"
                        ...
                        data-item-duration="{{ $service->duration_minutes }}">
                        <img src="{{ $service->imageUrl() }}" alt="{{ $service->name }}" class="rounded" style="width: 40px; height: 40px; object-fit: cover;">
                        <div class="fw-semibold small lh-sm">{{ $service->name }}</div>
                        <div class="text-secondary extra-small">${{ number_format($service->price, 2) }}</div>
                    </div>
                @endforeach
            </div>
        </div>
    ...
    <script>
        ...
        productSearch.addEventListener('input', () => {
            ...
            fetch(`{{ route('pos.search_products') }}?q=${query}`)
                .then(res => res.json())
        ...
        couponBtn.addEventListener('click', async () => {
            if (appliedCoupon) {
                ...
                if (couponCode) {
                    try {
                        const response = await fetch(`{{ url('/pos/validate-coupon') }}/${couponCode}`);
        ...
        confirmSaleBtn.addEventListener('click', async () => {
            ...
            if ((paymentMethod === 'tarjeta' || paymentMethod === 'mixto') && voucherFolioInput.value.trim() !== '') {
                ...
                const response = await fetch('{{ route('pos.check_voucher_folio') }}', {
                    method: 'POST',
                    headers: {
            ...
            .then(data => {
                if (data.success) {
                    window.location.href = `{{ url('/pos/pdf-preview') }}/${data.sale_id}`;
                } else {
