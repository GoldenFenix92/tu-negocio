...
<head>
    <meta charset="utf-8">
    <title>Reporte de Arqueo de Caja - {{ $cashCount->folio }}</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
        ...
        <table class="header-table">
            <tr>
                ...
                <td class="header-content">
                    ...
                    <div class="report-title">REPORTE DE ARQUEO DE CAJA</div>
                </td>
            </tr>
        </table>
    </div>
    ...
    <div class="info-section">
        <table class="info-table">
            <tr>
                <td class="info-label">Folio de Arqueo:</td>
                <td class="info-value">{{ $cashCount->folio }}</td>
                <td class="info-label">Fecha y Hora:</td>
                <td class="info-value">{{ $cashCount->created_at->format('d/m/Y H:i:s') }}</td>
            </tr>
            <tr>
                <td class="info-label">Usuario:</td>
                <td class="info-value">{{ $cashCount->user->name ?? 'N/A' }}</td>
                <td class="info-label">Efectivo Esperado:</td>
                <td class="info-value">${{ number_format($cashCount->expected_cash, 2) }}</td>
            </tr>
            <tr>
                <td class="info-label">Efectivo Real:</td>
                <td class="info-value">${{ number_format($cashCount->actual_cash, 2) }}</td>
                <td class="info-label">Diferencia:</td>
                <td class="info-value" style="color: {{ $cashCount->difference < 0 ? 'red' : ($cashCount->difference > 0 ? 'green' : 'black') }}">
                    ${{ number_format($cashCount->difference, 2) }}
                </td>
            </tr>
            <tr>
                <td class="info-label">Notas:</td>
                <td class="info-value" colspan="3">{{ $cashCount->notes ?? 'N/A' }}</td>
            </tr>
        </table>
    </div>
    ...
    <table class="data-table">
        ...
        <tbody>
            ...
            @empty
                <tr>
                    <td colspan="6" class="text-center">No hay ventas para mostrar en este arqueo.</td>
                </tr>
            @endforelse
        </tbody>
    </table>
